      ...
      {templates.map((temp) => (
        ...
        >
          ...
          <div className="aspect-video bg-[#faf8f5] border-b border-[#eae6df] flex flex-col items-center justify-center p-6 text-center relative group">
            {(temp.id === "aurelia" || temp.id === "eternal-journey") ? <Image src={temp.thumbnail} alt={`${temp.name} premium invitation`} fill sizes="(max-width: 768px) 90vw, 50vw" className="object-cover" /> : (<span className="font-serif italic font-bold text-3xl text-[#855f18]/60">{temp.name}</span>)}
